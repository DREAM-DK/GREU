...
# ==========================================================================
# Flow helpers  (nasa_10_nf_tr)
# ==========================================================================

"""Net flow (RECV − PAID) for na_item(s), grouped by (sector, year)."""
function net_direct_flow(df, items)
...
"""
function get_flow(df, items, flow_type, sectors = nothing)
  items_set = items isa AbstractString ? Set([items]) : Set(items)
  if flow_type == "NET"
    result = net_direct_flow(df, items_set)
  else
...
# ==========================================================================
# Balance sheet helpers  (nasa_10_f_bs)
# ==========================================================================
...
function build_parameters(flow_df, bal_df)
  return (;
    # ------------------------------------------------------------------
    ...
    vFinIncome = vcat([
      let d = get_flow(flow_df, items, dir); d.f .= f; d.al .= al; d end
    ...
    vNetFinTransactions            = get_flow(flow_df, "B9",           "RECV"),
    vNetTransfers2sector           = get_flow(flow_df, fin_transactions_transfer_items, "NET",  ["FinCorp", "NonFinCorp", "Hh"]),
    vGrossCapitalFormation         = get_flow(flow_df, "P5G",          "PAID", ["FinCorp", "NonFinCorp", "Hh"]),
    vGrossOpSurplusMixedIncome     = get_flow(flow_df, "B2A3G",        "RECV", ["FinCorp", "NonFinCorp"]),
    vNonFinancialNonProducedAssets = get_flow(flow_df, "NP",           "PAID", ["FinCorp", "NonFinCorp", "Hh", "RoW"]),

    # Households
    vHhConsumption           = select(get_flow(flow_df, "P3",               "PAID", ["Hh"]), :year, :value),
    vHhWages                 = select(get_flow(flow_df, "D1",               "RECV", ["Hh"]), :year, :value),
    vCorrectionNonFinCorp2Hh = select(get_flow(flow_df, "B2A3G_correction", "RECV", ["Hh"]), :year, :value),

    # Rest of World
    vRoWPrimaryIncomeCurrentBalanceOther = select(get_flow(flow_df, fin_transactions_row_other_items, "NET", ["RoW"]), :year, :value),
    vExports = select(get_flow(flow_df, "P6", "PAID", ["RoW"]), :year, :value),
    vImports = select(get_flow(flow_df, "P7", "RECV", ["RoW"]), :year, :value),
...
function refresh_sector_accounts_data!(dir = sector_accounts_data_dir)
  mkpath(dir)
  flow_df = process_sector_accounts(fetch_sector_accounts())
  bal_df  = process_fin_accounts_balance(fetch_fin_accounts_balance())
  params  = build_parameters(flow_df, bal_df)

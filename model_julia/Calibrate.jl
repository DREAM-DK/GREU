...
base_block = base_model(calibrated_modules)
...
static_calibration_block = sum(m.define_calibration() for m in calibrated_modules);
...
dynamic_calibration_block = sum(m.define_calibration() for m in calibrated_modules);

...
code_energy_accounts = 'env_ac_pefasu'
df = eurostat.get_data_df(code_energy_accounts,filter_pars={'startPeriod': year, 'endPeriod': year, 'geo': geo_list})
df.drop(columns=['freq','unit',r'geo\TIME_PERIOD'],inplace=True)
df.set_index(['stk_flow','nace_r2','prod_nrg'],inplace=True)
df.dropna(inplace=True)
df.reset_index(inplace=True)
df.to_excel(excel_writer='semig.xlsx')

...
$FUNCTION test_data():
  ...
  testvM[i,t] =  sum((es,e), Energybalance['base','imports',i,es,e,t]) 
              - sum(d, vIOE_m[i,d,t]);
  ABORT$(abs(sum((i,tData), testvM[i,tData]))>1) 'Test of energy-IO and energybalance failed! Value of imports do not match'; #£ Note the test tolerance is set relatively high at 1 bio. DKK. This should ideally be much lower, eg 1e-6
  ...
  ABORT$(abs(sum((d,tData), testvE_base[d,tData]))>1) 'Test of energy-IO and energybalance failed! Value of energy demand do not match'; #£ Note the test tolerance is set relatively high at 1 bio. DKK. This should ideally be much lower, eg 1e-6


  testvE_duties[d,t] = sum((es,e,demand_transaction_temp,etaxes), Energybalance[etaxes,demand_transaction_temp,d,es,e,t]) 
                    -  vIOE_a['TaxSub',d,t];

  ABORT$(abs(sum((d,tData), testvE_duties[d,tData]))>1) 'Test of energy-IO and energybalance failed! Value of energy duty revenues do not match'; #£ Note the test tolerance is set relatively high at 1 bio. DKK. This should ideally be much lower, eg 1e-6


  testvE_vat[d,t] = sum((es,e,demand_transaction_temp), Energybalance['VAT',demand_transaction_temp,d,es,e,t]) 
                  -  vIOE_a['Moms',d,t];

  ABORT$(abs(sum((d,tData), testvE_vat[d,tData]))>1) 'Test of energy-IO and energybalance failed! Value of VAT do not match'; #£ Note the test tolerance is set relatively high at 1 bio. DKK. This should ideally be much lower, eg 1e-6

"""Build docs/What_the_model_uses.xlsx — what GREU uses, and where it lives.

Three sheets: a half-page Read me; an everyday overview; then one row per
data-fed model variable, grouped by submodule. Only variables that
``@load`` from data.gdx are listed. Endogenous prices, j-terms and
behavioural parameters are out.
...
ROWS = [
    ...
    (
        ...
        "Not published in these categories; we construct the split",
    ...
    (
        ...
        "Money",
        "Eurostat investment by asset type; the industry split is coarser",
        "qI_k_i",
    ),
    (
        "Climate",
...
MODULE_ROWS = [
    ...
    (
        ...
        "National totals match; the industry split is coarser in many "
        "countries.",
    ),
    (
        "factor_demand",

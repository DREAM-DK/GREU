...
def load_data(n, t, country, currency, year_start, year_end, i_list=None, re_list=None, energy_list=None, **kwargs):
    # ========================================================================
    #   Define sets 
    # ========================================================================
    ...
    raw_data.rename(columns={'ind_ava': 'i', 'ind_use': 'd'}, inplace=True)
    raw_data = pd.melt(raw_data, id_vars=['i', 'd', 'stk_flow'],
                       value_vars=list(map(str, range(year_start - 1, year_end + 1))),
                       var_name='year', value_name='level')
    raw_data['level'] = raw_data['level'] / 1000
    
    # Rename demand components and aggregate industries by first letter
    ...
    raw_data = raw_data.groupby(['i', 'd', 'stk_flow', 'year'], as_index=False)['level'].sum()
    ...
    vIO_y = raw_data[(raw_data['stk_flow'] == 'DOM')
                    & raw_data['i'].isin(i_list)
                    & raw_data['d'].isin(d_list)][['i', 'd', 'year', 'level']]

    vIO_m = raw_data[(raw_data['stk_flow'] == 'IMP')
                    & raw_data['i'].isin(i_list)
                    & raw_data['d'].isin(d_list)][['i', 'd', 'year', 'level']]

    vIO_a = raw_data[(raw_data['stk_flow'] == 'TOTAL')
                    & raw_data['i'].isin(a_list)
                    & raw_data['d'].isin(d_list)][['i', 'd', 'year', 'level']]

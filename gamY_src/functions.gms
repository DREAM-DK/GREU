...
$FUNCTION load_dummies({time_subset}, {gdx}):
  SETS 
    ...
    load_d1vVirkNet[portf_,t]
    load_d1vOffAkt[portf_,t]
    load_d1vOffPas[portf_,t]
    load_d1vOffNet[portf_,t]
    load_d1Arv[a_,t]
    ;
  execute_load {gdx}
    load_d1IO=d1IO
    ...
    load_d1vVirkNet=d1vVirkNet
    load_d1vOffAkt=d1vOffAkt
    load_d1vOffPas=d1vOffPas
    load_d1vOffNet=d1vOffNet
    load_d1Arv=d1Arv
  ;
  ...
  d1vVirkNet[portf_,t]$({time_subset}[t]) = load_d1vVirkNet[portf_,t];
  d1vOffAkt[portf_,t]$({time_subset}[t]) = load_d1vOffAkt[portf_,t];
  d1vOffPas[portf_,t]$({time_subset}[t]) = load_d1vOffPas[portf_,t];
  d1vOffNet[portf_,t]$({time_subset}[t]) = load_d1vOffNet[portf_,t];
...
$FUNCTION unload({fname}):
  execute_unloaddi '{fname}'
    ...
    fpt, fqt, fvt, fp, fq, fv, INF_GROWTH_ADJUSTED, nOvf2Soc
    ...
    d1vHhAkt, d1vHhPas, d1vVirkAkt, d1vVirkPas, d1vVirkNet, d1vOffAkt, d1vOffPas, d1vOffNet, d1Arv
  ;
$ENDFUNCTION
...
$FUNCTION display_difference({group}, {suffix1}, {suffix2}):
  # Display differences for a group of variables (suffixes can be .l for current levels) 
  $offlisting
    $LOOP {group}:
...
$FUNCTION set_initial_levels_to_nonzero():
  ...
  $LOOP All:
    ...
    # If an endogenous variable still has a staring level of 0, set the starting level to the maximum absolute value of all periods
    {name}.l{sets}$(
           ({name}.up{sets} <> {name}.lo{sets})
